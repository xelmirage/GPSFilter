// GPSFilter.cpp : ¶¨Òå¿ØÖÆÌ¨Ó¦ÓÃ³ÌĞòµÄÈë¿Úµã¡£
//
...
struct dist
{
	...
	bool operator <(const dist& rhs) const // ÉıĞòÅÅĞòÊ±±ØĞëĞ´µÄº¯Êı
	{
		return distance < rhs.distance;
	}

	bool operator >(const dist& rhs) const // ½µĞòÅÅĞòÊ±±ØĞëĞ´µÄº¯Êı
	{
		return distance>  rhs.distance;
	}
...
POINTSL pointsL;//º½´ø
POINTSLV vector_pointsL;//º½´øÊı×é
DISTSV rows; //´æ´¢ºáÏòÅÅµÄÊı×é
DISTS row;
...
void LLtoUTM(double Long, double Lat) {
	...
	UTMNorthing = 0;
	UTMEasting = 0;
	double a = 6378137; // ÍÖÇò³¤°ëÖá
	double b = 6356752.3142451793; // ÍÖÇò¶Ì°ëÖá
	double f = (a - b) / a; // ±âÂÊ
	double FN = 0;
	// e±íÊ¾WGS84µÚÒ»Æ«ĞÄÂÊ,de±íÊ¾eµÄÆ½·½,
	double de = (2 * f - f * f);
	double k0 = 1;
	double ee = (de) / (1 - de);
	double N, T, C, A, M;

	// È·±£longtitudeÎ»ÓÚ-180.00----179.9Ö®¼ä
...
<<<<<<< HEAD
	...
	//DISTSV rows;
=======
>>>>>>> parent of 25f5b3c... åˆ°æ¯ä¸€èˆªå¸¦çš„å‚è¶³ï¼Œæ²¡æœ‰ç”¨è¿­ä»£
...
<<<<<<< HEAD
			...
			belt_out<<endl;
			belt_out<<images[i->id-1]<<" ";                          //ÖØÒª£º´Óimages[]Êı×éµ÷ÓÃÊ±Òª½«id-1£¬ÒòÎªÅÅidµÄÊ±ºòÃ»ÓĞ0
...
>>>>>>> parent of 25f5b3c... åˆ°æ¯ä¸€èˆªå¸¦çš„å‚è¶³ï¼Œæ²¡æœ‰ç”¨è¿­ä»£
	...
	{   
		std::cout << "ÕÒ²»µ½ÅäÖÃÎÄ¼şÄ¿Â¼,Çë¼ì²é¸ÃÄ¿Â¼ÊÇ·ñ´æÔÚ:"  ;
